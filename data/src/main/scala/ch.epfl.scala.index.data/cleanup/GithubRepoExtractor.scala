...
class GithubRepoExtractor(paths: DataPaths) {
  ...
  private val claims = resource.managed(Source.fromFile(paths.claims.toFile)).acquireAndGet {
    source => read[Claims](source.mkString).claims.filter(_.repo != void)
  }
  ...
  def updateClaims(): Unit = {
    ...
    val notClaimed = poms.filter(pom => apply(pom).isEmpty)
